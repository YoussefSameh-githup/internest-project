...
class SkillSourceForm(forms.Form):
    cv_file = forms.FileField(required=False, validators=_doc_validators, label="CV (PDF or DOCX)")
    linkedin_pdf = forms.FileField(required=False, validators=_doc_validators, label="LinkedIn profile PDF export")
    linkedin_url = forms.URLField(required=False, label="LinkedIn profile URL")
    partner_university = forms.ModelChoiceField(
        ...
        required=False,
        label="Share my skill analytics with my university",
        empty_label="— Do not share —",
    )
    ...
    def clean_linkedin_url(self):
        ...
        if url:
            ...
            if not (host == "linkedin.com" or host.endswith(".linkedin.com")):
                raise forms.ValidationError("Enter a linkedin.com profile URL.")
        return url

    def clean(self):
        data = super().clean()
        if not (data.get("cv_file") or data.get("linkedin_pdf") or data.get("linkedin_url")):
            raise forms.ValidationError("Upload your CV, your LinkedIn PDF export, or add your LinkedIn URL.")
        return data
